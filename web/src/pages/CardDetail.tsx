...
function Path({ potential }: { potential: EvoPotential }) { return <div className="detail-potential"><div className="detail-potential-head"><strong>{potential.final_overall} · GG {potential.final_gg_rating.toFixed(1)}</strong><Chip tone="gain">{formatSigned(potential.gg_rating_gain)} GG</Chip></div><div>{potential.coin_cost ? formatCoins(potential.coin_cost) : "grátis"}{potential.point_cost ? ` · ${potential.point_cost} pontos` : ""}</div><p>{potential.gained_play_styles?.length ? `Novos PlayStyles: ${styleNames(potential.gained_play_styles)}` : ""}</p></div>; }
...
function TransitionCard({ transition, graph, transitions, completed, onToggle }: { transition: EvolutionGraphTransition; graph: EvolutionGraph; transitions: EvolutionGraphTransition[]; completed: string[]; onToggle: (name: string, checked: boolean) => void }) {
  ...
  return <div className="detail-potential evo-transition"><div className="detail-potential-head"><strong>{from?.card.rating ?? "?"} → {to?.card.rating ?? "?"}{to?.card.gg_rating ? ` · GG ${to.card.gg_rating.toFixed(1)}` : ""}</strong><Chip tone={transition.is_expired ? "alert" : "coin"}>{transition.coin_cost ? formatCoins(transition.coin_cost) : "grátis"}</Chip></div><p className="evo-transition-name">{transition.evolution}</p><div className="evo-transition-badges">{isBranch && <Chip tone="flat">ramificação</Chip>}{isRejoin && <Chip tone="flat">reencontro</Chip>}{transition.lab && <Chip tone="flat">Lab</Chip>}{transition.repeatable && <Chip tone="flat">repetível</Chip>}{transition.is_expired && <Chip tone="alert">expirada</Chip>}</div><div>{transition.training_time || "tempo não informado"}{transition.point_cost ? ` · ${transition.point_cost} pontos` : ""}</div><label className="evo-transition-done"><input type="checkbox" checked={done} onChange={(event) => onToggle(transition.evolution, event.target.checked)} /> já concluí</label></div>;
}
