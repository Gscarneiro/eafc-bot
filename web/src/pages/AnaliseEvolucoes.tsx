...
import { evaluationSourceLabel, formatCoins, formatSigned, styleNames } from "../format";
...
function PathCard({ candidate, saved, onSave, saving }: { candidate: EvolutionPathCandidate; saved: boolean; onSave: () => void; saving: boolean }) {
  ...
  const currentScore = candidate.current_evaluation?.disponivel ? candidate.current_evaluation.nota : candidate.potential.final_gg_rating - candidate.potential.gg_rating_gain;
  const finalScore = candidate.final_evaluation?.disponivel ? candidate.final_evaluation.nota : candidate.potential.final_gg_rating;
  const scoreLabel = evaluationSourceLabel(candidate.final_evaluation?.contexto.fonte, true);
  const starterName = impact.starter?.common_name || impact.starter?.name;
  return <article className={`evo-route${impact.kind === "entra_no_xi" ? " is-xi" : ""}`}>
    ...
    <div className="evo-route-main">
      ...
      <div className="evo-route-outcome">
        <span><small>carta atual</small>{scoreLabel} {currentScore && currentScore > 0 ? currentScore.toFixed(1) : "—"}</span>
        <span aria-hidden="true">→</span>
        <span><small>final · {impact.position || final?.gg_rating_pos || final?.position || "—"}</small>{scoreLabel} {finalScore && finalScore > 0 ? finalScore.toFixed(1) : "—"}</span>
        {impact.kind !== "sem_comparacao" && <span className="evo-route-xi"><small>{impact.kind === "melhora_titular" ? `melhora ${starterName || "o titular"} · vaga ${(impact.slot_index ?? 0) + 1}` : `${impact.position} · entra no lugar de ${starterName || "titular não identificado"}`}</small>{formatSigned(impact.gain ?? 0)} {scoreLabel}</span>}
      </div>
      <div className="evo-route-meta"><span>{candidate.potential.coin_cost ? formatCoins(candidate.potential.coin_cost) : "grátis"}</span>{candidate.potential.point_cost > 0 && <span>{candidate.potential.point_cost} pontos</span>}{candidate.potential.training_time && <span>{candidate.potential.training_time}</span>}{candidate.potential.path.is_expired && <span className="evo-route-expired">expirada</span>}{candidate.potential.gained_play_styles?.length ? <span>{styleNames(candidate.potential.gained_play_styles)}</span> : null}{candidate.final_evaluation?.limitacoes?.slice(0, 1).map(item => <span key={item}>limitação: {item}</span>)}</div>
    </div>
...
export default function AnaliseEvolucoes() {
  ...
  if (gate) return gate;
  if (!data) return null;
  const scoreLabel = evaluationSourceLabel(data.avaliacao?.fonte);
  ...
  return <div className="wrap evo-analysis-page">
    <PageHeader eyebrow="evoluções" title="Análise do elenco" meta={`Todas as cartas OVR 88+ · decisão ordenada por ${scoreLabel}.`} />
    ...
    <div className="stat-grid evo-analysis-stats"><div className="stat-tile"><div className="label">cartas analisadas</div><div className="value">{summary.players}</div><div className="sub">OVR 88 ou mais</div></div><div className="stat-tile"><div className="label">com caminho</div><div className="value up">{summary.confirmed}</div><div className="sub">paths confirmados</div></div><div className="stat-tile"><div className="label">entram no XI</div><div className="value up">{summary.entra_no_xi}</div><div className="sub">{evaluationSourceLabel(data.avaliacao?.fonte, true)} posicional maior</div></div><div className="stat-tile"><div className="label">coleta pendente</div><div className="value down">{summary.fetch_error + summary.not_checked}</div><div className="sub">sem afirmação automática</div></div></div>
